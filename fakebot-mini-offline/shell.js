(() => {
  "use strict";
  const VERSION = "20260728-lofi-infinite";
  ...
  async function bootFrame(){
    ...
    try{
      ...
      await injectScript(doc, "fakebotMiniOfflineAudio", "offline-engine.js");
      watchPlayback();
      showStatus("Fakebot Mini Offline ready", 1100);
    }catch(err){
      console.error(err);
  ...
  async function runAction(action){
    ...
    if (action === "play"){
      ...
      try{
        if (!win.FakebotMiniSamples) throw new Error("Offline audio is not ready");
        showStatus("Starting offline audio…", 1800);
        ...
        showStatus(
          String(error?.message || "").includes("user gesture")
            ...
            : "Offline audio could not start. Tap Play again.",
          3400
        );
      }finally{
        playPending = false;
  ...
  window.addEventListener("message", (event)=>{
    ...
    if (event.data.type === "ui-ready") showStatus("Fakebot Mini Offline ready", 1000);
